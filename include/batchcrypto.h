...
typedef struct bc_stats {
  uint64_t calls, submitted, completed, item_errors, call_errors,
...
enum { BC_P256_REFERENCE = 0, BC_P256_COMB_W8 = 1, BC_P256_FULL_WINDOW_W8 = 2 };
/* Select a fixed-base implementation for signing AND public-key export.
 * Reference is the default. Compiled-in public G tables load lazily per
 * context. Selection serializes with calls, preserves keys/epochs, and never
 * falls back. Table memory is included in reserved_device_bytes. Requires
 * library >=0.2.0. */
BC_API int bc_set_p256_backend(bc_context *ctx, uint32_t backend);
BC_API int bc_get_p256_backend(bc_context *ctx, uint32_t *backend);
BC_API uint32_t bc_abi_version(void);
BC_API int bc_create(int device, bc_context **out);
/* Destroy requires callers to stop submitting work and join their threads
 * first. */
...
BC_API int bc_key_put(bc_context *ctx, uint32_t slot, uint32_t kind,
                      const uint8_t key[32], uint64_t expected_epoch,
                      uint64_t *new_epoch);
BC_API int bc_key_remove(bc_context *ctx, uint32_t slot,
                         uint64_t expected_epoch, uint64_t *new_epoch);
BC_API int bc_sha256_batch(int device, const bc_hash_item *items,
                           uint32_t count, uint8_t *digests, uint8_t *statuses);
BC_API int bc_p256_public_key(int device, const uint8_t key[32],
...
BC_API int bc_hash(bc_context *ctx, const bc_hash_item *items, uint32_t count,
                   uint8_t *digests, uint8_t *statuses,
                   bc_batch_report *report);
BC_API int bc_seal(bc_context *ctx, uint32_t slot, const bc_aead_item *items,
                   uint32_t count, uint8_t *statuses, bc_batch_report *report);
BC_API int bc_open(bc_context *ctx, uint32_t slot, const bc_aead_item *items,
                   uint32_t count, uint8_t *statuses, bc_batch_report *report);
 ...
 * Device index is explicit. Zero count succeeds without accessing other
 * arguments. On call-level error, outputs/statuses are unspecified: discard all
 * outputs. BC_OK means inspect each item status. Failed authentication zeros
 * that item's plaintext output. Each (key, nonce) MUST be unique for sealing
 * across calls, processes and restarts; the caller owns that lifecycle. No keys
 * are generated. Batch working data is limited to 64 MiB. */
BC_API int bc_aes256gcm_seal(int device, const uint8_t key[32],
                             const bc_aead_item *items, uint32_t count,
                             uint8_t *statuses);
BC_API int bc_aes256gcm_open(int device, const uint8_t key[32],
                             const bc_aead_item *items, uint32_t count,
                             uint8_t *statuses);
...
BC_API int bc_p256_sign(int device, const uint8_t private_key[32],
