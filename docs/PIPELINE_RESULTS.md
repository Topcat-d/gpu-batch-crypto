...
**New: [repeated RTX 3060 measurements](RTX3060_PIPELINE.md).** The follow-up uses an idle selected GPU with per-cell CPU/GPU telemetry on the shared host, opposite run order, longer checks and a separate 50 ms deadline control. It adds 32 rows while retaining all earlier data. Read its deadline-success fractions and actual GPU share alongside throughput.
...
They still provide an executable request pipeline, independent verification of every completed signature and inspectable overload/latency accounting. Harness preflight checks all GPUs by default for three observations at or below 5% utilization and refuses to start otherwise, unless the operator explicitly selects diagnostic contention. `--preflight-scope target-gpu` checks the selected device and explicitly qualifies the shared CPU host. Preflight repeats before every cell. The harness records per-second GPU utilization, memory, clocks, temperature, power and aggregate CPU counters. Quiet preflight is a useful control, not proof of an exclusive host or a measurement of wall-system energy.
...
The harness requires tracked source changes to be committed, refuses to overwrite a capture, checks quiet-GPU preflight before each cell, and bounds the run matrix. Use new filenames for repeats. Preserve every cell, including overload outcomes. `--allow-contended` explicitly labels diagnostic data; it does not turn it into an isolated comparison. To measure an idle second GPU while other work remains active, explicitly use `--device 1 --preflight-scope target-gpu` and retain the shared-host qualification. Windows CPU counters cover the whole host here (16 logical CPUs); on hosts exceeding 64 processors, GetSystemTimes covers the calling thread's primary processor group. Use `--gpu-dispatch owner` only as a separately recorded variant, with its extra dispatch thread and measured CPU usage included in the resource model.
