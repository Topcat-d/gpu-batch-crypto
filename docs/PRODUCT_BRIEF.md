...
## Implemented increment: make the simpler alternative adoptable

The prior comparison's hash-list alternative was benchmark-only. A new
experimental `batchcrypto.manifests` profile now serves a recipient receiving
...
Selection remains explicit. The new opener rejects Merkle receipts, and the old
...
Validation covers independent JWS verification, immutable handles, strict bounds
and profile separation, complete-export rejection, frozen vectors, both detached
formats and an installed wheel. The [actual API comparison](MANIFEST_API_RESULTS.md)
has 96 measured cells and 39,786 accepted record checks. At 1,024/full/shared,
manifest median total time was 9.543 ms versus Merkle 32.670 ms, with 76,306 versus
493,811 metadata bytes. At full/fresh consumption, validating a complete hash
list separately for every recipient was much slower. These outcomes support
explicit format choices, with no automatic routing or GPU-economics claim.
