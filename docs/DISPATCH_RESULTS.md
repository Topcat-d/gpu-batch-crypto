...
The captures include batches 1, 8, 64, 256, 1,024 and 4,096 across CPU, reference, comb and full-window paths: **96 total rows**, preserving every timing sample and correctness/accounting counter. These short captures establish the observed change, not a sustained service capacity guarantee or an independently replicated speedup. Exclusive-device telemetry was not archived for this earlier comparison. Background GPU work was discovered later during the separate pipeline campaign; its exact overlap with earlier captures is unknown. [Raw records and checksums](../benchmarks/dispatch_results).
