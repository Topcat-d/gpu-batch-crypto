...
def main():
    ...
    records = [
        json.dumps(
            {"record_id": i, "message": f"example-{i}"},
            sort_keys=True,
            separators=(",", ":"),
        ).encode()
        for i in range(32)
    ...
    with Runtime(a.library, a.device) as rt:
        rt.load_key(0, generate_p256_key(), "p256")
        public, epoch = rt.public_key(0)
        digests = rt.sha256(records)
        signatures = rt.sign(0, digests)
        for record, digest, sig in zip(records, digests, signatures):
            if digest != hashlib.sha256(record).digest() or not verify_p256(
                public, digest, sig
            ):
