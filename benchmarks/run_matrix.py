...
def main():
    ...
    p.add_argument("--sizes", default="16,512,1024,4096,16384,65536,262144,1048576")
    p.add_argument("--batches", default="1,8,64,256")
    a = p.parse_args()
    if a.iterations < 2:
        p.error("at least two measured iterations required")
    sizes = list(map(int, a.sizes.split(",")))
    batches = list(map(int, a.batches.split(",")))
    if any(not 0 <= s <= 1048576 for s in sizes) or any(
        not 1 <= n <= 4096 for n in batches
    ):
        p.error("size/batch outside API bounds")
    commit = command(["git", "rev-parse", "HEAD"])
    if command(["git", "diff", "--name-only", "HEAD"]):
        raise RuntimeError("commit tracked source changes before benchmarking")
    ...
    cuda_version = (
        command([compiler[1].strip(), "--version"]) if compiler else "unavailable"
    )
    config = []
    ...
    cpu_aes = AESGCM(aes_key)
    cpu_signer = ec.derive_private_key(int.from_bytes(sk, "big"), ec.SECP256R1())
    ...
    with ExitStack() as stack:
        ...
        cells = [
            ...
            for size in sizes
            for n in batches
        ] + [("p256_sign", 32, n) for n in sorted(set(batches + [1024, 4096]))]
        for op, size, n in cells:
                ...
                rows.append(
                    {
                        ...
                        "batch": n,
                        "status": "unsupported",
                        "reason": "64 MiB working-data limit",
                    }
                )
                continue
            ...
            for trial in range(a.iterations + 1):
                ...
                records = [
                    Record((nonce_counter + i).to_bytes(12, "big"), m, b"benchmark-v1")
                    for i, m in enumerate(data)
                ]
                nonce_counter += n
                if op == "aes256gcm_seal":
                    cpu = lambda: [
                        cpu_aes.encrypt(r.nonce, r.data, r.aad) for r in records
                    ]
                    gpu = lambda: rt.seal(0, records)
                elif op == "aes256gcm_open":
                    sealed = [
                        Record(r.nonce, cpu_aes.encrypt(r.nonce, r.data, r.aad), r.aad)
                        for r in records
                    ]
                    cpu = lambda: [
                        cpu_aes.decrypt(r.nonce, r.data, r.aad) for r in sealed
                    ]
                    gpu = lambda: rt.open(0, sealed)
                elif op == "sha256":
                    cpu = lambda: [hashlib.sha256(m).digest() for m in data]
                    gpu = lambda: rt.sha256(data)
                else:
                    cpu = lambda: cpu_sign(data)
                    gpu = lambda: rt.sign(1, data)
                results = {}
                # Alternate order to reduce systematic warm-cache/order bias.
                order = (
                    (("cpu", cpu), ("cuda", gpu))
                    if trial % 2
                    else (("cuda", gpu), ("cpu", cpu))
                )
                ...
                if results["cuda"] != results["cpu"]:
                    raise RuntimeError(f"correctness mismatch: {op}/{size}/{n}")
                if rt.last_report["completed"] != n or rt.last_report["errors"]:
            ...
            for name, times in timings.items():
                ...
                row = {
                    ...
                    "backend": name,
                    "payload_bytes": size,
                    "aad_bytes": 12 if op.startswith("aes") else 0,
                    "batch": n,
                    "iterations": a.iterations,
                    "total_operations": n * a.iterations,
                    "total_seconds": total,
                    "batch_seconds": times,
                    "median_batch_ms": statistics.median(times) * 1000,
                    "ops_per_second": n * a.iterations / total,
                    "plaintext_bytes_per_second": n * a.iterations * size / total
                    if op.startswith("aes")
                    else None,
                    "input_bytes_per_second": n * a.iterations * size / total,
                    "correctness": "all measured outputs match independent CPU result",
